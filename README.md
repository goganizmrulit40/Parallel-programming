...
## 6. Выводы
- Успешный вход в систему и настройка окружения.
- Компиляция MPI-программы: Использован Intel MPI Compiler с оптимизацией -O3.
- Работа с Slurm: Написан и запущен скрипт submit_job.sh, задача успешно выполнена на узлах n[239-240].
- Получены экспериментальные результаты: Для матриц размером 200–2000 получены времена выполнения, ускорения и эффективность.
- Масштабируемость: Параллельная версия стабильно даёт ускорение 19–53×, что подтверждает эффективность MPI-подхода.
